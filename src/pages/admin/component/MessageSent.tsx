...
function MessageSent({setMsgDetail, setMessageData} : msg) {
  ...
  const [activeDropdown, setActiveDropdown] = useState<number | null>(null);
     ...
     useEffect(() => {
      ...
      const handleEmail = async(pageNumber : number) => {
          ...
          try {
            const response = await fetch(`${baseUrl}/message-sent?page=${pageNumber}`, requestOptions);
            if (!response.ok) {
     ...
     const handleMessage = ($data : messageIntern) => {
      ...
      setMessageData([$data]);
       ...
       const handleDateFilter = async(e: React.ChangeEvent<HTMLSelectElement>) => {
             ...
             try {
               const response = await fetch(`${baseUrl}/filter-message/sent/${startDate.toISOString()}/${endDate.toISOString()}`, requestOptions);
               if (!response.ok) {
                 const errorResponse = await response.json();
                 throw new Error(errorResponse.message);
                 }
       ...
       const handleDateSearch = async(value : string) => { 
            if(value){
             ...
             try {
               const response = await fetch(`${baseUrl}/filter-message/sent/${value}`, requestOptions);
               if (!response.ok) {
                 const errorResponse = await response.json();
                 throw new Error(errorResponse.message);
                 }
