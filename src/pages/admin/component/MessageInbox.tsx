...
const MessageInbox = ({setMsgDetail, setMessageData} : msg) => {
...
const [activeDropdown, setActiveDropdown] = useState<number | null>(null);
   ...
   useEffect(() => {
    ...
    const handleEmail = async(pageNumber : number) => {
        ...
        try {
          const response = await fetch(`${baseUrl}/message-inbox?page=${pageNumber}`, requestOptions);
          if (!response.ok) {
  ...
  const handleDateFilter = async(e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        try {
          const response = await fetch(`${baseUrl}/filter-message/inbox/${startDate.toISOString()}/${endDate.toISOString()}`, requestOptions);
          if (!response.ok) {
            const errorResponse = await response.json();
            throw new Error(errorResponse.message);
            }
  ...
  const handleDateSearch = async(value : string) => { 
       if(value){
        ...
        try {
          const response = await fetch(`${baseUrl}/filter-message/inbox/${value}`, requestOptions);
          if (!response.ok) {
            const errorResponse = await response.json();
            throw new Error(errorResponse.message);
            }
                              ...
                              {activeDropdown === index && (
                                <div className="dropdown-menu">
                                  {/* <div onClick={() => handleMove(item.id, "inbox")}>Move to Inbox</div> */}
                                   {
                                          item.is_read ? (
                                          <div onClick={() => handleMove(item.id, "unread")}>Mark as Unread</div>
                                          ) : (
