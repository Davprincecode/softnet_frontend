...
function MessageFavourite({setMsgDetail, setMessageData} : msg) {
  ...
  const [activeDropdown, setActiveDropdown] = useState<number | null>(null);
     ...
     useEffect(() => {
      ...
      const handleEmail = async(pageNumber : number) => {
          ...
          try {
            const response = await fetch(`${baseUrl}/message-favorite?page=${pageNumber}`, requestOptions);
            if (!response.ok) {
     ...
     const handleFavourite = async(id : string) => {
      ...
      setMessageData([$data]);
       ...
       const handleDateFilter = async(e: React.ChangeEvent<HTMLSelectElement>) => {
             ...
             try {
               const response = await fetch(`${baseUrl}/filter-message/favorite/${startDate.toISOString()}/${endDate.toISOString()}`, requestOptions);
               if (!response.ok) {
                 const errorResponse = await response.json();
                 throw new Error(errorResponse.message);
                 }
       ...
       const handleDateSearch = async(value : string) => { 
            if(value){
             ...
             try {
               const response = await fetch(`${baseUrl}/filter-message/favorite/${value}`, requestOptions);
               if (!response.ok) {
                 const errorResponse = await response.json();
                 throw new Error(errorResponse.message);
                 }
