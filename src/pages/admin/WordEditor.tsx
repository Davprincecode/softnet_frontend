...
// const WordEditor: React.FC<EditPop> = ({ editorRef }) => {
const WordEditor  = ({ onChange, initialContent}: EditPop) => {
...
useEffect(() => { 
 ...
 const insertImageFromUrl = (url?: string) => {
  ...
  // Add resizers
  ["top-left","top-right","bottom-left","bottom-right","top","bottom","left","right"].forEach(pos => {
...
// ---------------------------------------------------------------------------------------------------------------
  /* -----------------------
    ...
    ------------------------ */
      ...
      ------------------------------*/
      if (e.key === "Backspace") {
      // Cursor is directly on an image
      if (node.nodeName === "IMG" && node instanceof HTMLElement) {
      e.preventDefault();
      node.remove();
      return;
      }

      // Cursor is after an image (text node, offset 0)
      if (node.nodeType === Node.TEXT_NODE && range.startOffset === 0) {
      const prev = node.previousSibling;
      if (prev && prev.nodeName === "IMG" && prev instanceof HTMLElement) {
      e.preventDefault();
      prev.remove();
      return;
      }
      ...
      // Cursor is before an image (text node)
      if (node.nodeType === Node.TEXT_NODE) {
      const next = node.nextSibling;
      if (next && next.nodeName === "IMG" && next instanceof HTMLElement) {
      e.preventDefault();
      next.remove();
      return;
      }
      ...
      currentP = (node as Element).closest("p");
      } else if (node.parentElement) {
      currentP = node.parentElement.closest("p");
      }
      ...
      // Cut content **after cursor**
      const afterRange = range.cloneRange();
      afterRange.setStart(range.startContainer, range.startOffset);
      afterRange.setEndAfter(currentP.lastChild!);
      ...
      if (newP.firstChild) {
      // Place cursor before the <br> or text
      newRange.setStart(newP.firstChild, 0);
      } else {
      // Fallback: place inside <p>
